...
boolean WiFiManager::autoConnect() {
  String ssid = "SHP6-" + String(ESP.getChipId());
...
boolean WiFiManager::startConfigPortal() {
  String ssid = "Shellly-" + String(ESP.getChipId());
...
boolean  WiFiManager::startConfigPortal(char const *apName) {
  ...
  DEBUG_WM(_apName);
  String ssid = "SHP6-" + String(WiFi.macAddress());
  delay(100);
  _apName = ssid.c_str();
...
void WiFiManager::handleReset() {
  ...
  page += _customHeadElement;
  page += FPSTR(HTTP_HEAD_END);
  page += F("SHP6 startet in wenigen Sekunden neu...");
  page += FPSTR(HTTP_END);
  server->sendHeader("Content-Length", String(page.length()));
  server->send(200, "text/html", page);
