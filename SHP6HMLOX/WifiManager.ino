bool doWifiConnect() {
  ...
  const char* ipStr = SHP6NetConfig.ip; byte ipBytes[4]; parseBytes(ipStr, '.', ipBytes, 4, 10);
  const char* netmaskStr = SHP6NetConfig.netmask; byte netmaskBytes[4]; parseBytes(netmaskStr, '.', netmaskBytes, 4, 10);
  const char* gwStr = SHP6NetConfig.gw; byte gwBytes[4]; parseBytes(gwStr, '.', gwBytes, 4, 10);
  ...
  if (!startWifiManager && _ssid != "" && _psk != "" ) {
    ...
    if (String(SHP6NetConfig.ip) != "0.0.0.0") {
    ...
    WiFiManagerParameter custom_shp6name("shp6", "SHP6 Ger&auml;tename", GlobalConfig.DeviceName, VARIABLESIZE, 0, "pattern='[A-Za-z0-9_ -]+'");
    ...
    WiFiManagerParameter custom_ip("custom_ip", "IP-Adresse", (String(SHP6NetConfig.ip) != "0.0.0.0") ? SHP6NetConfig.ip : "", IPSIZE, 0, "pattern='((^|\\.)((25[0-5])|(2[0-4]\\d)|(1\\d\\d)|([1-9]?\\d))){4}$'");
    WiFiManagerParameter custom_netmask("custom_netmask", "Netzmaske", (String(SHP6NetConfig.netmask) != "0.0.0.0") ? SHP6NetConfig.netmask : "", IPSIZE, 0, "pattern='((^|\\.)((25[0-5])|(2[0-4]\\d)|(1\\d\\d)|([1-9]?\\d))){4}$'");
    WiFiManagerParameter custom_gw("custom_gw", "Gateway",  (String(SHP6NetConfig.gw) != "0.0.0.0") ? SHP6NetConfig.gw : "", IPSIZE, 0, "pattern='((^|\\.)((25[0-5])|(2[0-4]\\d)|(1\\d\\d)|([1-9]?\\d))){4}$'");
    ...
    wifiManager.addParameter(&custom_shp6name);
    ...
    DEBUG("CUSTOM STATIC IP: " + String(SHP6NetConfig.ip) + " Netmask: " + String(SHP6NetConfig.netmask) + " GW: " + String(SHP6NetConfig.gw));
    if (wm_shouldSaveConfig) {
      if (String(custom_ip.getValue()).length() > 5) {
        DEBUG("Custom IP Address is set!");
        strcpy(SHP6NetConfig.ip, custom_ip.getValue());
        strcpy(SHP6NetConfig.netmask, custom_netmask.getValue());
        strcpy(SHP6NetConfig.gw, custom_gw.getValue());

      } else {
        strcpy(SHP6NetConfig.ip,      "0.0.0.0");
        strcpy(SHP6NetConfig.netmask, "0.0.0.0");
        strcpy(SHP6NetConfig.gw,      "0.0.0.0");
      }
      ...
      strcpy(GlobalConfig.DeviceName, custom_shp6name.getValue());
